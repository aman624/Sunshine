...
public class WeatherProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        int rowsDeleted = 0;
        if (selection == null) {
            // database.delete mentions this to get count if all rows are to be deleted
            selection = "1";
        }
        switch (match) {
            case WEATHER:
    ...
    public int update(
        ...
        switch (match) {
            case WEATHER:
                normalizeDate(values);
                rowsUpdated = database.update(WeatherContract.WeatherEntry.TABLE_NAME,
                        values,
                        selection,
                        selectionArgs);
                break;
            case LOCATION:
                rowsUpdated = database.update(WeatherContract.LocationEntry.TABLE_NAME,
                        values,
                        selection,
                        selectionArgs);
                break;
